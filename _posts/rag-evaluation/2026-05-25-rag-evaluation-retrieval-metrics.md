---
title: "RAG Evaluation Part 1: The Pipeline and Retrieval Metrics"
date: 2026-05-25 10:00:00 +0000
categories: [RAG Evaluation]
tags: [RAG, Evaluation, RAGAS, DeepEval, Retrieval Metrics, NDCG, Information Retrieval]
mermaid: true
math: true
---
...
Getting that grip requires understanding where quality can break down. A RAG pipeline has a retriever and a generator, and quality can degrade at either stage or in the relationship between them. This post covers the pipeline structure and the three evaluation surfaces it creates (Section 1), then dives into retrieval metrics: classical information retrieval metrics with their annotation costs and production tradeoffs (Section 2), and how modern frameworks (RAGAS, DeepEval) automate retrieval evaluation using LLM judges (Sections 3-4). Faithfulness and answer relevance metrics are covered in [Part 2]({% post_url rag-evaluation/2026-05-25-rag-evaluation-faithfulness-and-relevance %}).
...
This post focuses on **Edge 1 (retrieval quality)** in depth. The remainder covers classical metrics (Section 2), then how RAGAS (Section 3) and DeepEval (Section 4) automate retrieval evaluation with LLM judges.
...
These metrics were developed for web search ranking and document retrieval, where the same problem exists: given a query, surface the most relevant results. RAG inherits both the metrics and their limitations from that lineage.
...
### 3.3 How RAGAS Differs from Classical Metrics
...
**Inputs:** query, retrieval_context (no response or reference answer needed)

**What it measures:** Is the retrieved context relevant to the query?

**How it works:**
1. For each retrieved chunk independently, the LLM extracts **statements** (high-level information units) from that chunk.
2. For each extracted statement, the LLM judges whether it is relevant to the input query.
3. Score = relevant statements / total statements across all chunks.
...
Retrieved chunks:
- Chunk 1: "The Eiffel Tower was completed in 1889 for the World's Fair. It stands 330 meters tall."
  - Statement 1: "The Eiffel Tower was completed in 1889 for the World's Fair." → relevant (answers when)
  - Statement 2: "It stands 330 meters tall." → not relevant (height, not year)
- Chunk 2: "Gustave Eiffel also designed the internal structure of the Statue of Liberty."
  - Statement 1: "Gustave Eiffel designed the internal structure of the Statue of Liberty." → not relevant

Score = 1/3 = 0.33

**Key difference from RAGAS Context Precision:** DeepEval judges relevance to **the query** (no reference answer needed). RAGAS judges relevance to **the reference answer** (requires ground truth). This makes DeepEval's Contextual Relevancy suitable for production monitoring on live traffic where no reference answer exists.

### 4.2 Contextual Precision

**Inputs:** query, retrieval_context (order matters), expected_output (reference answer)

**What it measures:** Are the relevant chunks ranked higher than the irrelevant ones?

**How it works:**
1. In a single LLM call, the judge evaluates all retrieved chunks in order, determining for each whether it was "remotely useful in arriving at the expected output."
2. Each chunk receives a binary verdict (yes/no) with a required reason.
3. Score is computed as weighted average precision (MAP-style):
...
score = weighted_sum / relevant_count
```

This rewards relevant chunks appearing at earlier positions. If all relevant chunks are at the top, score = 1.0. If they are scattered or at the bottom, the score drops.

**Key difference from RAGAS Context Precision:** The computation is nearly identical (both compute Average Precision against LLM-judged relevance to the reference answer). The main difference is implementation: DeepEval evaluates all chunks in a single LLM call, while RAGAS evaluates each chunk independently.

### 4.3 Contextual Recall

**Inputs:** query, retrieval_context, expected_output (reference answer)

**What it measures:** Does the retrieved context contain enough information to produce the expected answer?

**How it works:**
1. The LLM decomposes the expected_output into individual sentences.
2. For each sentence, it judges whether that sentence "can be attributed to any parts of the retrieval context."
3. Each verdict requires a reason that cites specific nodes and quotes from the context.
4. Score = attributable sentences / total sentences.

$$\text{Contextual Recall} = \frac{\text{Sentences in expected output attributed to context}}{\text{Total sentences in expected output}}$$

**Key difference from RAGAS Context Recall:** Functionally equivalent. Both decompose the reference answer into sentences, check attribution against context, and compute the ratio. The implementation details differ (DeepEval requires the LLM to cite specific node numbers and quotes in its reasoning), but the metric concept and formula are the same.

### 4.4 Summary: RAGAS vs. DeepEval Retrieval Metrics

| Metric | RAGAS Equivalent | Reference Answer Required? | Key Difference |
|--------|-----------------|---------------------------|----------------|
| Contextual Relevancy | (no direct equivalent) | No | Evaluates relevance to query, not to reference answer. Usable for online monitoring. |
| Contextual Precision | Context Precision | Yes | Same concept (MAP). DeepEval uses single LLM call for all chunks. |
| Contextual Recall | Context Recall | Yes | Same concept. DeepEval requires citation of specific nodes in reasoning. |
